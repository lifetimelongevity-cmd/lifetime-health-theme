...
- Einlösung des Willkommens-Deals: **Code angelegt und AKTIV** — `WILLKOMMEN10`
  ...
  **ERLEDIGT: Mail-Block ist eingebaut (BJ, 24.07. abends)** — nur WILLKOMMEN10, das Dossier ist
  zurückgestellt (Perk 4 entsprechend gekürzt); Dossier-Zeile in Perk + Mail erst ergänzen, wenn
  die PDF-Zustellung steht. Damit ist die Einlösekette komplett: PDP-Perk → Code → Mail.
  Zweiter Push 24.07. abends ist live verifiziert (Störer, Klarna-Pill, Perk 4).
